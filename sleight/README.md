# Sleight v7.4 — trackpad in the air
...
```bash
cd "/Users/mohamwedayoub/app Idea/sleight" && ./run.sh --train
```
...
```bash
cd "/Users/mohamwedayoub/app Idea/sleight" && ./run.sh
```
...
Sleight lives as a **frosted glass widget floating at the bottom of your screen** — no window, no title bar, transparent background, black/white/grey only. It never takes focus and never eats a click, so it can't get in the way of the app you're controlling.
...
If macOS won't provide the floating window, Sleight falls back to a plain dark panel automatically rather than failing to start.
...
**System Settings → Privacy & Security → Accessibility → enable your terminal app**, then restart Sleight. Until then everything runs but no events land (DRY-RUN badge shows).
...
`sleight_config.json`: `cursor_deadzone` (idle-drift immunity, default 0.009 — raise if the cursor still creeps, lower for finer aiming), `scroll_gain` (px per hand-width, default 900), `scroll_flip`, `swipe_left_key`/`swipe_right_key` (`back`, `forward`, arrow or page keys — defaults are real browser Back/Forward; dangerous keys are rejected), `panel_pos` (`center` or `right`, also the `p` key).

## Safety rails

Clean modifier state on all synthetic events (a held Cmd can't corrupt them) · no irreversible actions mappable · click needs a reaching index (a fist can never click) · guarded dry-run toggle that can't lie about being LIVE · every session logs per-arm records to `sleight_log_*.jsonl`.
